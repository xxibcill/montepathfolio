...
| Release 7 — trading and microstructure | Exact-transition OU simulation and OLS fit, deterministic price-time-priority event replay, per-event accounting snapshots, optional maker/taker fees, bounded ladders, illustrative single-asset risk-budget and other agent rules, fee-inclusive wealth history, and numerically stable analytical Almgren-Chriss schedules are in [`trading.ts`](../src/lib/quant/trading.ts) and [`trading.test.ts`](../src/lib/quant/trading.test.ts). | **Partial: genuine risk parity and venue coupling are deferred.** The current book trades one asset, so it intentionally exposes a risk-budget rule rather than mislabeling it risk parity. Release 7 becomes complete only after a multi-asset venue can submit and account for true risk-parity orders. The book remains a stylized deterministic scenario, not a calibrated venue or evidence of a profitable strategy. |
